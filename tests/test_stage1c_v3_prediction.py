...
def _pair(
    ...
    return build_prospective_pair(
        ...
        runtime_fingerprint="gemma3-v3-test-runtime",
        epsilon=1.0e-12,
        tolerance=1.0e-9,
    )
...
def test_v3_pair_id_is_explicitly_namespaced_and_score_is_baseline_only() -> None:
    ...
    payload = {
        ...
        "seed": "stage1c-v3-preregistered-prospective-prediction",
        "source": [0, 1, 3],
        "target": [2, 1, 8],
    }
    assert (
...
def test_selection_does_not_embed_an_endpoint_overlap_blocker() -> None:
    pair = _pair(_source(14, 234), _target(15, 771), response=-3.0)
    selected = select_pair_groups((pair,), selection=_selection(), tolerance=1.0e-9)
    assert selected.primary == (pair,)
