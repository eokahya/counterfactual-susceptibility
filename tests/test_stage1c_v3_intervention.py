...
from cfsus.stage1c_v3.intervention_runtime import _feature
...
def test_v3_intervention_rejects_bool_feature_coordinates_and_uses_dynamic_length() -> (
    None
):
    with pytest.raises(ScientificInputError, match="feature record"):
        _feature({"layer": True, "position": 1, "feature_id": 2}, "source")
    source = inspect.getsource(
        __import__("cfsus.stage1c_v3.intervention_runtime", fromlist=["x"])
    )
