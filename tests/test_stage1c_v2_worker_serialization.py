"""Synthetic regression tests for the v2 worker lifetime boundary."""
...
def test_strict_write_read_preserves_pair_ids_and_point_counts(tmp_path: Path) -> None:
    result = build_detached_worker_result(
        _sweeps(), artifact_type="stage1c_v2_intervention_worker"
    )
    path = tmp_path / "worker.json"
    write_json_new(path, result)
    reread = read_json_strict(path)
    assert isinstance(reread, dict)
    assert [item["pair_id"] for item in reread["sweeps"]] == ["a" * 64, "b" * 64]
    assert [item["point_count"] for item in reread["sweeps"]] == [2, 2]
    assert (
        reread["sweeps"]
        == reread["intervention_artifacts"]["intervention_sweeps"]["pairs"]
    )
