"""Frozen pair-level analyses and scientific outcome classification."""
...
def analyze_pair(
    ...
    for item in nonzero:
        ...
        if predicted_delta > 0.0:
            signs.append(observed_delta > 0.0)
    ...
    full = next(
        (item for item in ordered if float(item["realized_suppression"]) == 1.0),
        None,
    )
    if full is None:
    ...
    directional_violation = group == "directional" and full_delta > 0.0
    near_boundary_crossing = group == "near_boundary" and any(
        bool(item["target_active"]) for item in ordered
    )
    active_seen = False
    ...
    return {
        ...
        "directional_control_violation": directional_violation,
        "near_boundary_control_crossing": near_boundary_crossing,
        "nonmonotonic_gate": nonmonotonic_gate,
    }


def classify_outcome(pair_analyses: list[dict[str, Any]]) -> ScientificOutcome:
    """Apply the frozen outcome precedence after a valid canonical runtime."""
