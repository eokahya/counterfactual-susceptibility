#!/usr/bin/env python3
"""Execute exactly one frozen Stage 1C-v2 intervention attempt.

The worker accepts only the committed v2 prediction manifest.  It performs
...
from cfsus.stage1c_v2.analysis import (  # noqa: E402
    aggregate_analyses,
    analyze_pair,
    classify_outcome,
)
from cfsus.stage1c_v2.config import (  # noqa: E402
    BASE_COMMIT,
    BRANCH,
    CONFIG_PATH,
    EXPERIMENT_CLASS,
...
from cfsus.stage1c_v2.intervention import (  # noqa: E402
    bisection_requested_alpha,
    plan_applied_values,
)
from cfsus.stage1c_v2.intervention_runtime import (  # noqa: E402
    Stage1CInterventionBackend,
)
from cfsus.stage1c_v2.prediction import canonical_v2_pair_id  # noqa: E402
from cfsus.stage1c_v2.serialization import (  # noqa: E402
    read_json_strict,
    write_json_new,
)
from cfsus.stage1c_v2.worker_result import (  # noqa: E402
...
def _protocol_hashes() -> dict[str, str]:
    paths = (
        ...
        "src/cfsus/stage1c_v2/config.py",
        "src/cfsus/stage1c_v2/prediction.py",
        "src/cfsus/stage1c_v2/vjp.py",
        "src/cfsus/stage1c_v2/runtime.py",
        "src/cfsus/stage1c_v2/intervention.py",
        "src/cfsus/stage1c_v2/intervention_runtime.py",
        "src/cfsus/stage1c_v2/analysis.py",
        "src/cfsus/stage1c_v2/serialization.py",
        "src/cfsus/stage1c_v2/worker_result.py",
        "scripts/stage1c_v2/preflight_stage1c_v2.py",
        "scripts/stage1c_v2/run_stage1c_v2.py",
        "scripts/stage1c_v2/run_stage1c_v2_prediction_worker.py",
        "scripts/stage1c_v2/run_stage1c_v2_intervention_worker.py",
        "scripts/stage1c_v2/assemble_stage1c_prediction.py",
        "scripts/stage1c_v2/assemble_stage1c_artifacts.py",
        "scripts/stage1c_v2/validate_stage1c_v2_artifacts.py",
    )
...
def _feature(value: Any, label: str, *, token_count: int) -> FeatureRef:
    ...
    if not (0 <= result.layer < 18 and 1 <= result.position < token_count):
        raise RuntimeError(f"{label} is outside the selected v2 prompt positions")
...
def _verify_tracked_prediction_manifest(path: Path) -> None:
    ...
    if path.resolve() != expected.resolve() or path.is_symlink() or not path.is_file():
        raise RuntimeError("worker must read the tracked v2 prediction manifest")
    try:
        ...
        raise RuntimeError(
            "tracked v2 prediction manifest is absent from HEAD"
        ) from error
...
def _validate_prediction_manifest(
    ...
    if (
        ...
        or manifest.get("artifact_type") != "stage1c_v2_prediction_manifest"
        or manifest.get("experiment_class") != EXPERIMENT_CLASS
        or manifest.get("status") != "prediction_frozen_ready_for_commit"
    ):
        raise RuntimeError("prediction manifest v2 identity or status is invalid")
    ...
    if (
        ...
        raise RuntimeError("prediction protocol differs from frozen v2 config")
    if int(config["intervention"]["canonical_attempts"]) != 1:
        raise RuntimeError("v2 canonical intervention attempts must equal one")
    ...
    if not isinstance(groups, dict) or set(groups) != expected_groups:
        raise RuntimeError("selected v2 groups are malformed")
    ...
    for group in ("primary", "near_boundary", "directional"):
        ...
        for raw in rows:
            ...
            if pair_id != canonical_v2_pair_id(
                source=source,
                target=target,
                ...
                raise RuntimeError(
                    "selected pair ID differs from the frozen v2 identity"
                )
...
def _artifact_bundle(
    ...
    return {
        ...
        "crossing_summary": {
            "artifact_type": "stage1c_v2_crossing_summary",
            "pairs": analyses,
            "aggregate_metrics": aggregates,
            "scientific_outcome": outcome,
        },
        "local_linearity_summary": {
            "artifact_type": "stage1c_v2_local_linearity_summary",
            "pairs": analyses,
            "aggregate_metrics": aggregates,
            "scientific_outcome": outcome,
        },
...
def _verify_assets(cache: Path) -> dict[str, Any]:
    ...
    if record.get("status") != "verified":
        raise RuntimeError("immutable v2 asset verification failed")
    return {
        "status": "verified",
...
def _execute(args: argparse.Namespace) -> dict[str, Any]:
    config = load_stage1c_v2_config(args.config, require_token_ids=True)
    assert_fallback_disabled()
    if (
        os.environ.get("HF_HUB_OFFLINE") != "1"
        or os.environ.get("TRANSFORMERS_OFFLINE") != "1"
    ):
        raise RuntimeError("v2 intervention worker requires offline mode")
    from preflight_stage1c_v2 import verify_git
    ...
    try:
        ...
        if telemetry["violations"] or telemetry["telemetry_failures"]:
            raise RuntimeError("v2 intervention telemetry contains a safety failure")
        git_end = verify_git("intervention", args.pre_intervention_commit)
        if git_end != git_start:
            raise RuntimeError("v2 worktree identity changed during intervention")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        raise RuntimeError(
            "v2 intervention output paths must be new with existing parents"
        )
