#!/usr/bin/env python3
"""Bounded offline supervisor for the Stage 1C-v2 workers."""
...
SCRIPT_ROOT = REPOSITORY_ROOT / "scripts" / "stage1c_v2"
...
from cfsus.stage1c_v2.config import CONFIG_PATH, load_stage1c_v2_config  # noqa: E402
from cfsus.stage1c_v2.serialization import write_json_new  # noqa: E402

try:
    from preflight_stage1c_v2 import CREDENTIAL_VARIABLES
...
def _validate_output_directory(path: Path, config: dict[str, Any]) -> Path:
    if (
        ...
        raise RuntimeError(
            "v2 output directory must be an empty absolute real directory"
        )
    ...
    if not (
        ...
        or (
            ...
            and resolved.name.startswith("stage1c-v2-")
        )
    ):
        raise RuntimeError("v2 output directory is outside the frozen generated roots")
    return resolved


def _worker_command(args: argparse.Namespace, output_dir: Path) -> list[str]:
    worker_name = (
        "run_stage1c_v2_prediction_worker.py"
        if args.phase == "prediction"
        else "run_stage1c_v2_intervention_worker.py"
    )
    worker = SCRIPT_ROOT / worker_name
    if worker.is_symlink() or not worker.is_file():
        raise RuntimeError("v2 worker is missing or unsafe")
    command = [
        sys.executable,
        str(worker),
        "--config",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if preflight_record.get("status") != "passed":
        raise RuntimeError("v2 preflight did not pass")
    ...
    write_json_new(
        ...
        {
            "schema_version": 2,
            "artifact_type": "stage1c_v2_supervisor",
            "phase": args.phase,
            **supervisor,
        },
    )
    ...
    if (
        ...
        raise RuntimeError("v2 worker failed under bounded supervisor")
    result = json.loads(worker_output.read_text(encoding="utf-8"))
    if result.get("status") != "passed":
        raise RuntimeError("v2 worker result did not pass")
