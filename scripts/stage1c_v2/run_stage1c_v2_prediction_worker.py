#!/usr/bin/env python3
"""Run the baseline-only Stage 1C-v2 prediction phase.

This worker has no intervention-worker import and does not read any historical
prediction or result artifact. Runtime/model code is imported only after the
offline and exact Git/config gates pass. Token IDs and scan positions must
match the immutable tokenizer identity frozen by preflight.
"""
...
from cfsus.stage1c_v2.prediction import (  # noqa: E402
...
from cfsus.stage1c_v2.runtime import Stage1CPredictionBackend  # noqa: E402
...
def _execute(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("v2 prediction worker requires offline mode")
    git_start = _git_identity()
    if git_start["branch"] != BRANCH or git_start.get("working_tree_clean") is not True:
        raise RuntimeError("v2 prediction worker requires the clean protocol commit")
    ...
    try:
        ...
        with sampler.stage("prediction_active_source_pool"):
            ...
            if eligible_pair_count > int(config["responses"]["maximum_eligible_pairs"]):
                raise RuntimeError("v2 eligible pair pool exceeds the frozen cap")
        ...
        if len(pairs) != eligible_pair_count:
            raise RuntimeError("v2 eligible pair enumeration is incomplete")
        selected_pairs = (
            *selected.primary,
            *selected.near_boundary,
            *selected.directional,
        )
        assert_v2_selection_disjoint_from_v1(selected_pairs)
        ...
        if telemetry["violations"] or telemetry["telemetry_failures"]:
            raise RuntimeError("v2 prediction telemetry contains a safety failure")
        git_end = _git_identity()
        if git_end != git_start:
            raise RuntimeError("v2 prediction worktree identity changed")
        manifest = {
            "schema_version": 2,
            ...
            "branch": BRANCH,
            "pair_id_domain": f"{EXPERIMENT_CLASS}:capital_germany_heldout_v2",
            "runtime_identity": {
                "backend": "nnsight",
                "device": "mps:0",
        ...
        if not isinstance(detached, dict):  # pragma: no cover
            raise SerializationError("v2 prediction manifest is not an object")
        return {
            "schema_version": 2,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.output.exists() or args.emergency_output.exists():
        raise RuntimeError("v2 prediction output paths must be new")
    if not args.output.parent.is_dir() or not args.emergency_output.parent.is_dir():
        raise RuntimeError("v2 prediction output parents must exist")
