#!/usr/bin/env python3
"""Thin Stage 1F orchestration over the accepted production runtime."""
...
from cfsus.stage1d.rehearsal import run_rehearsal, validate_rehearsal  # noqa: E402
from cfsus.stage1f import (  # noqa: E402
    CONFIG_PATH,
    EXPERIMENT_CLASS,
...
def _prediction_worker(args: argparse.Namespace) -> None:
    ...
    if protocol["protocol_commit"] != git["head"]:
        raise RuntimeError("prediction must run at the pushed Stage 1F protocol commit")
    model: Any = None
    sampler: Any = None
    finished = False
    try:
        ...
        prediction = build_prediction_manifest(
            model, torch, sampler, config, protocol=protocol, git=git
        )
        telemetry = sampler.finish()
        finished = True
        if telemetry["violations"] or telemetry["telemetry_failures"]:
            raise RuntimeError(
                "Stage 1F prediction telemetry contains a safety failure"
            )
        write_json_new(
            output,
            {
                "schema_version": 1,
                "artifact_type": "stage1f_prediction_worker",
                "status": "passed",
...
def _assemble_prediction(args: argparse.Namespace) -> None:
    ...
    validate_protocol(ROOT, protocol, config)
    if (
        worker.get("artifact_type") != "stage1f_prediction_worker"
        or worker.get("status") != "passed"
    ):
        raise RuntimeError("Stage 1F prediction worker did not pass")
    ...
    if prediction["prediction_execution_commit"] != git["head"]:
        raise RuntimeError("Stage 1F prediction worker commit differs")
    write_json_new(output, prediction)
    print(
        json.dumps({"status": "passed", "sha256": sha256_file(output)}, sort_keys=True)
    )
...
def _verify_tracked_prediction(path: Path) -> None:
    ...
    if tracked != path.read_bytes():
        raise RuntimeError(
            "working Stage 1F prediction differs from tracked HEAD bytes"
        )
...
def _rehearse(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        rehearsal_id = record["worker"]["sweeps"][0]["pair_id"]
        if rehearsal_id in evaluation_ids:
            raise RuntimeError("real active-feature rehearsal overlaps Stage 1F panel")
        telemetry = sampler.finish()
        finished = True
        if telemetry["violations"] or telemetry["telemetry_failures"]:
            raise RuntimeError("Stage 1F rehearsal telemetry contains a safety failure")
    finally:
        if sampler is not None:
            _finish_sampler(sampler, already_finished=finished)
        if model is not None:
...
def _intervention_worker(args: argparse.Namespace) -> None:
    ...
    if not pairs:
        raise RuntimeError("Stage 1F frozen panel is empty")
    prediction_sha = sha256_file(prediction_path)
    if journal_path.exists() or attempt_lock.exists() or output.exists():
        raise RuntimeError("Stage 1F canonical output, journal, and lock must be new")
    model: Any = None
    sampler: Any = None
    finished = False
    with CanonicalExecutionJournal(
        ...
        attempt_lock_artifact_type="stage1f_local_scientific_attempt_lock",
    ) as journal:
        try:
            model, torch, sampler, environment, runtime_evidence = _load_runtime(
                cache, config, emergency
            ...
            sweeps, calls = execute_frozen_pairs(
                ...
                maximum_bisection_steps=int(
                    config["refinement"]["maximum_bisection_steps"]
                ),
                stop_bracket_width=float(config["refinement"]["stop_bracket_width"]),
            )
            telemetry = sampler.finish()
            finished = True
            if telemetry["violations"] or telemetry["telemetry_failures"]:
                raise RuntimeError(
                    "Stage 1F canonical telemetry contains a safety failure"
                )
            write_json_new(
                output,
                {
                    "schema_version": 1,
                    "artifact_type": "stage1f_intervention_worker",
                    "status": "passed",
                    "prediction_manifest_sha256": prediction_sha,
...
def _assemble(args: argparse.Namespace) -> None:
    ...
    validate_prediction(prediction, config, protocol)
    if (
        worker.get("artifact_type") != "stage1f_intervention_worker"
        or worker.get("status") != "passed"
    ):
        raise RuntimeError("Stage 1F intervention worker did not pass")
    if worker.get("journal_sha256") != sha256_file(journal):
        raise RuntimeError("Stage 1F journal digest differs from worker evidence")
    points = read_completed_journal(journal)
    if worker.get("instrumented_source_suppression_api_calls") != len(points):
        raise RuntimeError("Stage 1F worker call count differs from journal")
