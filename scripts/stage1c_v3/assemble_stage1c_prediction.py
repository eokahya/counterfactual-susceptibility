#!/usr/bin/env python3
"""Publish the detached Stage 1C-v3 prediction manifest.
...
def assemble_prediction(
    ...
    *,
    ...
    if worker.get("schema_version") != 3:
        raise ValueError("prediction worker is not v3")
    if worker.get("artifact_type") != "stage1c_v3_prediction_worker":
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        raise RuntimeError("prediction assembly requires a clean v3 protocol commit")
    _git("merge-base", "--is-ancestor", BASE_COMMIT, head)
    if (
        _git("rev-parse", f"refs/remotes/origin/{BRANCH}") != head
        or _git("rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}")
        != f"origin/{BRANCH}"
    ):
        raise RuntimeError("prediction assembly requires the pushed protocol commit")
    expected = (
        ROOT
    ...
    if args.output.absolute() != expected:
        raise RuntimeError("prediction output path differs from the frozen v3 path")
