#!/usr/bin/env python3
"""Assemble and independently validate the compact Stage 1C-v3 bundle.

This module consumes only detached JSON worker records.  It has no model or
intervention imports.  Every JSON artifact is published with the v3 strict
...
import validate_stage1c_v3_artifacts as validator  # noqa: E402
...
def _require_worker(value: dict[str, Any], artifact_type: str) -> None:
    if value.get("schema_version") != 3 or value.get("artifact_type") != artifact_type:
...
def _asset_manifest(raw: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        "schema_version": 3,
        "artifact_type": "stage1c_v3_asset_manifest",
        ...
        "authentication_value_recorded": False,
        "exact_allowlist_hashes_verified": True,
        "actual_total_bytes": raw["actual_total_bytes"],
        "model": {
            "identifier": "google/gemma-3-270m",
            "revision": raw["model_revision"],
            "total_bytes": raw["model_total_bytes"],
        },
        "transcoder": {
            "identifier": "mwhanna/gemma-scope-2-270m-pt",
            "revision": raw["transcoder_revision"],
            "subfolder": "transcoder_all/width_16k_l0_small",
            "layer_count": 18,
            "total_bytes": raw["transcoder_total_bytes"],
        },
    }
...
def _environment_manifest(raw: dict[str, Any] | None, execution: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": 3,
        "artifact_type": "stage1c_v3_environment_manifest",
        "experiment_class": validator.EXPERIMENT_CLASS,
        "status": "passed",
        ...
        "packages": {
            "circuit-tracer": raw["circuit-tracer"],
            "torch": raw["torch"],
            "nnsight": raw["nnsight"],
        ...
        "accelerator": {
            ...
            "mps_built": raw["mps_built"],
            "mps_available": raw["mps_available"],
            "fallback_variable_present": raw["fallback_variable_present"],
            "outer_autocast_enabled": raw["outer_autocast_enabled"],
...
def records(
    ...
    _require_worker(prediction_worker, "stage1c_v3_prediction_worker")
    _require_worker(intervention_worker, "stage1c_v3_intervention_worker")
    ...
    if (
        intervention_worker.get("instrumented_source_suppression_api_calls")
        != point_count
    ):
        raise ValueError("instrumented worker API-call count differs from points")
    ...
    if attempts_raw.get("scientific_retry_count", 0) != 0 or attempts_raw.get(
        ...
        raise ValueError("worker attempts metadata violates v3 policy")
    ...
    local = _summary("stage1c_v3_local_linearity_summary", analyses, aggregate, outcome)
    errors = [
        validator.number(
    ...
    crossing = _summary("stage1c_v3_crossing_summary", analyses, aggregate, outcome)
    run = {
        "schema_version": 3,
...
def write_bundle(result: dict[str, dict[str, Any]], output_dir: Path) -> None:
    ...
    if set(result) != set(ALLOWLIST):
        raise ValueError("assembled records differ from exact v3 allowlist")
